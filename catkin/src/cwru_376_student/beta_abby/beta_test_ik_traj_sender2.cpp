...
int main(int argc, char** argv) 
{
    ...
    std::vector<Vectorq6x1> q6dof_solns; // instantiate a vector class which has element type Vectorq6x1
    Vectorq6x1 qvec;
    ...
    b_des << 1,0,0; //Comma-initialization Matrix and vector coefficients can be conveniently set using the so-called comma-initializer syntax. 
    t_des << 0,1,0;
    n_des = t_des.cross(b_des); //cross product t_des x b_des
   ...
   a_tool_des.linear() = R_des; // initialize the orientation part of 4-by-4 a_tool_des matrix as same as R-des
